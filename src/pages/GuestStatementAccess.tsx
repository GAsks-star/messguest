...
export default function GuestStatementAccess() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`[Pernyataan Tamu] Memulai pencarian server-side untuk: "${guestName}" pada tanggal ${checkInDate}`);
      ...
      try {
        ...
        console.warn('[Pernyataan Tamu] Gagal query terfilter, mencoba scan broad sebagai fallback...', innerError);
        try {
          snapshot = await getDocs(submissionsRef);
        } catch (broadError) {
          console.error('[Pernyataan Tamu] Gagal total membaca data.', broadError);
          throw broadError;
        }
      }
      
      console.log(`[Pernyataan Tamu] Berhasil membaca ${snapshot.size} dokumen.`);
      setIsOffline(false);
      ...
      console.log(`[Pernyataan Tamu] Ditemukan ${matched.length} data yang cocok dengan nama.`);
      ...
      if (approved.length > 0) {
        ...
        if (latest.id && estimatedTime) {
          try {
            ...
            console.warn('Failed to update estimated time, but continuing to form:', updateErr);
          }
        }
      ...
      console.error('[Pernyataan Tamu] Error Detail:', error);
      ...
      if (errorMessage.includes('Failed to get documents from server') || 
        ...
        setIsOffline(true);
        toast.error('Sistem kesulitan terhubung ke server database. Kami sedang mencoba memulihkan koneksi. Silakan klik "Segarkan Koneksi" jika masalah berlanjut.');
        ...
        console.error('Security Rules mungkin memblokir akses atau data belum ada.');
      } else {
        toast.error(`Akses Database Bermasalah: ${errorMessage}`);
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans selection:bg-primary/10">
              ...
              <CardContent className="p-8 md:p-12">
                ...
                <form onSubmit={handleSearch} className="space-y-8">
                  ...
                  <div className="p-6 border border-slate-100 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50">
                    ...
                    <Button variant="outline" className="rounded-xl border-slate-200 hover:bg-white hover:border-primary hover:text-primary font-bold text-xs gap-2 h-10 px-5 transition-all">
      ...
      <footer className="py-12 text-center">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
          © 2024 Mess Stay. Semua hak dilindungi.
        </p>
      </footer>
    </div>
  );
